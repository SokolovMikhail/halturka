...
?>
<div class="mb-20 mt-10" data-service-list="">
	<h2 class="mt-0">Услуги</h2>
	<button type="button" class="btn btn-success mb-10" data-service-add="warranty">Добавить услугу</button>
	<div class="row">
		<div class="col-md-3">
...
<h3 class="">Техника</h3>
<a href="/admin/main-devices/create-warranty/?orderId=<?= $model->id?>"><button type="button" class="btn btn-success mb-10">Добавить технику</button></a>
					
<? if(isset($result['items'][1]) && count($result['items'][1])){?>
	...
	<table class="table table-hover personal-task mb-0">
		<tbody>
			<tr class="personal-task_head">
				...
				<td style="max-width: 200px;">Проблема</td>
			...
			<? 
			$i=0;
			foreach ($result['items'][1] as $item){
				$i++;?>
				<tr id="item-<?= $item['id']?>">
					<td>
					<?=$i?>
					...
					<td>
						<a href="/admin/main-devices/update-warranty/?id=<?= $item['id']?>&orderId=<?= $model->id?>" class="<?= (isset($item['red']) || !isset($item['brands'])) ? 'text-red' : ''?>"><?= isset($item['brands']) ? $item['brands'] : 'Модель не выбрана'?></a>
					</td>
					<td>
					<?= $item['year']?>
					</td>								
					...
					<td class="ta-c">
						<a href="/admin/main-devices/create-warranty/?orderId=<?=$model->id?>&deviceId=<?=$item['id']?>"><span class="fa fa-files-o copy-icon" aria-hidden="true" data-toggle="tooltip" data-placement="top" title="Копировать"></span></a>
					...
					<td>
						<span class="<?= $item['warranty_result'] ? 'green-clr' : 'red-clr'?> fa fa-wrench fa-1" 
							aria-hidden="true"
							data-toggle="tooltip" 
							data-placement="top" 
							title="" 
							data-original-title="<?= $item['warranty_result'] ? 'Есть отчет' : 'Нет отчета'?>"							
							>
						</span>
					</td>					
				</tr>
...
<?if($equipmentArray['config']){?>
	...
	<?}?>
<?}else{?>
	<a href="/admin/configuration/create/?brandId=<?= $model->id?>&ownerType=order"><button type="button" class="btn btn-primary mb-10">Редактирование списка оборудования</button></a>
<?}?>
<div class="row mt-10">
	<div class="col-md-12">
		<?= $form->field($model, 'comments_warranty')->textArea(['maxlength' => 2048, 'rows' => 5]) ?>
	</div>
</div>